...
- **Accessibility:** the panel reuses Board's `wwwroot/js/focus-trap.js` (`focusTrap.activate`/`deactivate`) so Tab cannot leave the open palette, and focus is restored to the pre-open element on close. The input is a `role="combobox"` (`aria-controls`/`aria-expanded`/`aria-autocomplete="list"`/`aria-activedescendant`) over a `role="listbox"` result list whose rows are `role="option"` + `aria-selected` — arrow keys move `aria-activedescendant` rather than DOM focus, which is what makes the selection audible to screen readers. `focus-trap.js` keeps a **stack** of traps precisely because `Ctrl+K` is global and can open the palette on top of an already-trapped dialog (Board's add-note dialog): the palette pushes a trap and releases it on close, restoring the dialog underneath rather than leaving it untrapped. Release is LIFO, which holds as long as a nested dialog visually covers the one below it.
...
Frontend-only connectivity awareness (issue #211) — no backend, schema, or dependency change. Deliberately **not** an offline-first PWA: there is no service worker and no read-side caching, so pages still need the network to load. The scope is (1) telling the user they are offline and (2) not losing a save made while they are.

- **Detection:** `wwwroot/js/network-status.js` (`window.networkStatus`) wraps `navigator.onLine` plus the window `online`/`offline` events; `NetworkStatusService` exposes `IsOnline` + an `OnChanged` event and is initialized once from `MainLayout`. `navigator.onLine` reports link-layer state, not API reachability (it reads `true` behind a captive portal or a down server), so it is used **only to classify a failure that already happened** — a save is always attempted first and parked only once it actually fails. Never gate a request on it.
- **Banner:** `Components/OfflineBanner.razor`, hosted once in `MainLayout` above the page grid, `role="status"` + `aria-live="polite"`.
- **Save queue:** `PendingSaveStore` keeps failed-while-offline saves in `localStorage` — `localStorage`, not `DraftStore`'s `sessionStorage`, so a queued save survives the tab closing, which is exactly when work would be lost. It is **one entry per note, latest-wins**, stored under its **own key** (`vanadium.pending-save.v1.{noteId}`): a note save is a full-document PUT, so the newest edit supersedes any earlier queued edit for that note. The per-note key is a correctness requirement, not a layout choice — a single JSON-array key forced every enqueue/removal into a lock-free read-modify-write across three interop round-trips, so two interleaved writers could write back a list that silently dropped the other's entry. Keep every queue mutation a **single** `setItem`/`removeItem`; key enumeration goes through `offlineStore.valuesWithPrefix` (`wwwroot/js/offline-store.js`). Entries carry `ParentNoteId` (a PUT without it would re-parent the note to root) and the `BaseUpdatedAt` they were made against, so the server's optimistic-concurrency check still applies on replay. Only notes that already exist are queued — a never-saved note has no id and replaying it could duplicate it (that case stays with `DraftStore`).
- **Flush ownership:** `OfflineSaveQueue.FlushAsync` runs from `MainLayout` on reconnect (and at startup), so a save parked before the user navigated away still lands. An open `NoteEditor` **claims** its note and the flush **skips claimed notes**, because the live editor holds fresher content and re-saves the note itself on reconnect — without the claim both would PUT the same note and the loser would take a 409 and show a conflict banner for an edit nobody else touched. Claims live in `localStorage` via `NoteClaimStore` (`vanadium.note-claim.v1.{noteId}`), **not** in memory, because the queue is shared by every tab on the origin: an in-memory claim only excluded the flush in the *same* tab. A claim stores an **expiry** the owning editor refreshes on a 15 s heartbeat (45 s TTL) so a crashed tab cannot own a note forever. The invariant is *exactly one writer per note across all tabs* — if you add another writer for notes, give it the same claim/release treatment.
- **Claim ⇄ queue handoff:** a claim would otherwise **starve** the claimed note's queue entry (the flush skips it, and an editor that loaded the server copy has no pending changes to re-save — then the next successful save's `DiscardAsync` deletes an offline edit that never landed). So an editor opening a note calls `OfflineSaveQueue.AdoptAsync`, merging any parked entry into its own state and riding its own save; the entry stays in the store until that save succeeds, so closing the tab first still leaves it for a later flush. `DiscardAsync` is only safe **because** of this pairing — do not drop an entry from a writer that has not adopted it. A parked entry whose `BaseUpdatedAt` is older than the loaded server version is stale and is dropped (same rule as a 409).
- On replay a 409/403/404 **drops** the entry (the note moved on without it; retrying could only clobber newer content or fail forever); any other failure keeps it for the next flush. Drops are counted in `OfflineFlushResult.Dropped` and surfaced as a warning snackbar — discarding offline work must never be log-only. `NoteService.SaveAsync` maps 404 explicitly so this rule can actually fire. The flush is skipped when unauthenticated, since a 401 would bounce the user to `/login`, and it is guarded against re-entrancy (startup flush and a spurious `online` event can coincide).
- **Init order:** `NetworkStatusService.InitAsync` runs in `Program.cs` **before the first render**, not from `MainLayout`. Initializing it later leaves a window in which the service still reports its optimistic `IsOnline = true` default, and a save failing in that window is misclassified as a generic error and never queued.
