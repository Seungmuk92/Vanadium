...
- **Frontend**: `NotePropertyPanel` (editor, between title and Tiptap body — pure Blazor, no `tiptapInterop`), `PropertyValueEditor`, `PropertyFilterBar` (Home), `/properties` management page. Web `PropertyService` caches definitions per circuit (invalidated after any definition/option mutation).
