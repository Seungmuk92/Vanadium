...
A third note state between active and deleted: `NoteItem.ArchivedAt` (+ `IsArchiveRoot` on the directly-archived note). Archived notes are kept forever (no purge job by design), hidden from Home/Board/children/mention search, included in full-text search (`NoteSummary.IsArchived` drives the UI badge), retrievable via `GET /api/notes/{id}`, and READ-ONLY: `PUT /api/notes/{id}` and property value write/clear return 403 (`ProblemDetails`), checked before the optimistic-concurrency check. Creating or re-parenting under an archived note is rejected with 400. Archiving sweeps active descendants into a shared-`ArchivedAt` group (mirrors `DeletedAt` groups); unarchive restores the same group and re-attaches to root if the original parent is missing, soft-deleted, or still archived.
...
- **Storage is EAV, not JSONB** (decision §4.1): one `NotePropertyValues` row per (note, definition) with one nullable typed column each (`TextValue`/`NumberValue`/`DateValue`/`BoolValue`/`SelectedOptionId`), plus a `NotePropertySelectedOptions` join table for MultiSelect. It uses composite PKs, matching soft-delete query filters and DB cascades, and keeps every filter/sort translatable + SQLite-testable. A composite alternate key `PropertyOption (DefinitionId, Id)` makes "option belongs to its definition" (INV-P3) a DB guarantee.
...
- **INV-P4 `IgnoreQueryFilters()` rule**: `NotePropertyValue`/`NotePropertySelectedOption` carry `DeletedAt == null` query filters mirroring `NoteItem`'s. **Any definition-level scan — type-change guard, usage counts, MultiSelect empty-row cleanup — MUST use `IgnoreQueryFilters()`** or it undercounts recycle-bin notes' values and breaks lossless restore (see `PropertyService`). Note *value* endpoints use the default-filtered set (a recycle-bin note is invisible → 404).
- **Values are server-owned (INV-P5)**: never read from the `POST/PUT /api/notes` payload (`Properties` is `[NotMapped]`, `PropertyValues` is `[JsonIgnore]`). The dedicated `PUT/DELETE /api/notes/{noteId}/properties/{definitionId}` endpoints are the only mutation path, and they do **not** bump `NoteItem.UpdatedAt` (same reasoning as `SetShare`).
- **Read model**: `NoteItem.Properties` / `NoteSummary.Properties` (both `[NotMapped]`/projected) carry non-empty values ordered by definition `SortOrder`, populated by the service layer.
...
- **Frontend**: `NotePropertyPanel` (editor, between title and Tiptap body — pure Blazor, no `tiptapInterop`), `PropertyValueEditor`, `PropertyFilterBar` (Home/Board), `PropertyValueFormatter` (shared list-row/board-card display strings), `/properties` management page. Web `PropertyService` caches definitions per circuit (invalidated after any definition/option mutation).
- **Board grouping**: `/board` groups notes by a **Select** definition — one column per `PropertyOption`, plus a "No value" column for notes whose value row is absent (INV-P1). That column is not a drop *target* — it carries no `data-option-id`, so dropping onto it can never clear a value — but its cards ARE drag *sources*: dragging one onto an option column assigns that option (#375), and since such a card has no `data-option-id` either, the drag reports a null source option (`OnDropFromJs`/`MoveNoteToOptionAsync` take `Guid?`). **Clearing a value is done from the card's ⋮ move sheet, never by a drop** — its "No value" entry calls `DELETE /api/notes/{id}/properties/{definitionId}`. That entry is the only way to unset the grouping property from the board, because once every note has a value the "No value" column is not rendered at all and there is nothing to drag a card back into. MultiSelect is deliberately not groupable: a note would appear in several columns at once with no single column to move it out of. A drag/move is ONE `PUT /api/notes/{id}/properties/{definitionId}` with the target `OptionId` — because a Select holds exactly one option, that single write both empties the old column and fills the new one, so a mid-move failure can never strip the value and drop the note out of every column. `Board.razor` loads the unpaged `GET /api/notes/summaries` (not the paged list) so notes with no value are present client-side for the "No value" column; server-side filtering on the grouping property would hide them entirely.
...
| `/board` | `Board.razor` — kanban board, columns = a Select property's options |
...
- **New API endpoint:** add (1) DTO in `Vanadium.Note.REST/Models`, (2) DTO mirror in `Vanadium.Note.Web/Models`, (3) HTTP client method in `NoteService`/`PropertyService`/etc.
...
- **Keyboard shortcuts / slash commands — always update the Ctrl+/ help.** The keyboard-shortcuts help overlay in `Vanadium.Note.Web/Layout/MainLayout.razor` is the single user-facing catalog of shortcuts, and it does NOT auto-discover editor-internal keys or slash commands — those are listed by hand in the `EditorKeys` and `SlashCommands` static arrays. Whenever you add, remove, or change any of the following, update the matching entry in `MainLayout.razor` in the **same** change so the help never drifts:
  - a global shortcut registered via `KeyboardShortcutService` (these auto-render from `RegisteredShortcuts`, but confirm the `Description` reads well);
  - an editor keymap in `wwwroot/js/tiptap/extensions/toggle-keymap.js` or `tab-indent.js` (e.g. `Mod-Enter`, `Tab`, `Enter`/`Backspace` toggle behavior) → update `EditorKeys`;
  - a slash command in `SLASH_COMMANDS` in `wwwroot/js/tiptap/extensions/slash-commands.js` → update `SlashCommands`.
