...
- **Save queue:** `PendingSaveStore` keeps failed-while-offline saves in `localStorage` (key `vanadium.pending-saves.v1`) — `localStorage`, not `DraftStore`'s `sessionStorage`, so a queued save survives the tab closing, which is exactly when work would be lost. It is **one entry per note, latest-wins**: a note save is a full-document PUT, so the newest edit supersedes any earlier queued edit for that note. Entries carry `ParentNoteId` (a PUT without it would re-parent the note to root) and the `BaseUpdatedAt` they were made against, so the server's optimistic-concurrency check still applies on replay. Only notes that already exist are queued — a never-saved note has no id and replaying it could duplicate it (that case stays with `DraftStore`).
- **Flush ownership:** `OfflineSaveQueue.FlushAsync` runs from `MainLayout` on reconnect (and at startup), so a save parked before the user navigated away still lands. An open `NoteEditor` **claims** its note and the flush **skips claimed notes**, because the live editor holds fresher content and re-saves the note itself on reconnect — without the claim both would PUT the same note and the loser would take a 409 and show a conflict banner for an edit nobody else touched. If you add another writer for notes, give it the same claim/release treatment. On replay a 409/403/404 **drops** the entry (the note moved on without it; retrying could only clobber newer content or fail forever); any other failure keeps it for the next flush. The flush is skipped when unauthenticated, since a 401 would bounce the user to `/login`.
