...
- **Check existing reviews.** If a previous review from this same workflow already exists on the PR (recognizable by the 요약/완료 조건 점검 structure in its `body`):
...
- **Acceptance criteria** — go through each **완료 조건** item and mark it met / not met / unverifiable from the diff.
...
- **Approve** — only when **every** 완료 조건 item is **met** (none not-met, none unverifiable), scope is clean, and conventions/security/tests pass.
- **Comment** — no blocking problems, but either (a) one or more 완료 조건 items are **unverifiable** from the diff and available verification, or (b) there are non-trivial observations worth surfacing before merge. **An unverifiable acceptance criterion caps the verdict at Comment — never Approve.**

## 5. Post the review to GitHub

Write the review body in **Korean** (matching the repo's issue/PR language), structured as:

- **요약** — one-paragraph verdict and why. If this is a re-review after new commits (step 0), say so.
- **완료 조건 점검** — checklist of each acceptance criterion with met / not-met / unverifiable.
